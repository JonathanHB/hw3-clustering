    ...
    def score(self, X: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        for i, xi in enumerate(X):
            for j, xj in enumerate(X[i:]):
                distances[i][j+i] = np.dot(xi-xj, xi-xj)
        ...
        for i in range(n):
            ...
            intradists1 = [j for x, j in enumerate(distances[i][i+1:]) if y[x+i+1] == y[i]]
            ...
            #collect distances across the ith row
            interdists1 = [j for x, j in enumerate(distances[i][i+1:]) if y[x+i+1] != y[i]]
            #collect distances down the ith column
            if i == 0:
                interdists2 = []
            elif i == 1:
...
def _clustering_test_plot(k, seed, kmseed):
    ...
    sil = Silhouette()
    silscore = sil.score(np.array(cl_in[0]), np.array(cl_in[1]))
